...
contract CrossChainNameServiceReceiver is CCIPReceiver {
    ...
    constructor(address router, address lookup, uint64 sourceChainSelector) CCIPReceiver(router) {
    ...
    function _ccipReceive(Client.Any2EVMMessage memory message)
        internal
        override
        onlyFromSourceChain(message.sourceChainSelector)
    {
        (string memory _name, address _address) = abi.decode(message.data, (string, address));
